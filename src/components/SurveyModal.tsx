  ...
  return (
    <div className="fixed inset-0 z-[10000] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm transition-all duration-300">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto flex flex-col relative animate-fade-in-up">
        {/* Header */}
        <div className="bg-green-800 text-white p-5 sticky top-0 z-10 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-green-100 text-xs">Kelurahan Putussibau Kota</p>
            </div>
          </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="bg-gray-50 p-4 rounded-xl border border-gray-100 space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input type="text" name="nama" value={formData.nama} onChange={handleChange} placeholder="Biarkan kosong jika ingin anonim" className="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 outline-none" />
                  </div>
                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      <label className="block text-xs font-semibold text-gray-600 mb-1">Umur</label>
                      <input type="number" name="umur" value={formData.umur} onChange={handleChange} placeholder="Tahun" className="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 outline-none" />
                    </div>
                    <div>
                      <label className="block text-xs font-semibold text-gray-600 mb-1">Jenis Kelamin</label>
                      <select name="jenis_kelamin" value={formData.jenis_kelamin} onChange={handleChange} className="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 outline-none bg-white">
              ...
              <div className="space-y-5">
                <div>
                  ...
                  <select name="jenis_layanan" required value={formData.jenis_layanan} onChange={handleChange} className="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 outline-none bg-white">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  <div className="bg-white p-4 rounded-xl border border-gray-200">
                    ...
                    <div className="space-y-2">
                      {['Sangat Baik', 'Baik', 'Cukup', 'Kurang'].map(val => (
                        <label key={`prosedur-${val}`} className={`flex items-center p-2 rounded cursor-pointer border transition ${formData.kemudahan_prosedur === val ? 'bg-green-50 border-green-200' : 'border-transparent hover:bg-gray-50'}`}>
                          <input type="radio" name="kemudahan_prosedur" value={val} checked={formData.kemudahan_prosedur === val} onChange={handleChange} className="mr-3 text-green-600 focus:ring-green-500" required />
                  ...
                  <div className="bg-white p-4 rounded-xl border border-gray-200">
                    ...
                    <div className="space-y-2">
                      {['Sangat Baik', 'Baik', 'Cukup', 'Kurang'].map(val => (
                        <label key={`kecepatan-${val}`} className={`flex items-center p-2 rounded cursor-pointer border transition ${formData.kecepatan_pelayanan === val ? 'bg-green-50 border-green-200' : 'border-transparent hover:bg-gray-50'}`}>
                          <input type="radio" name="kecepatan_pelayanan" value={val} checked={formData.kecepatan_pelayanan === val} onChange={handleChange} className="mr-3 text-green-600 focus:ring-green-500" required />
                          <span className="text-sm text-gray-700">{val}</span>
                        </label>
                      ))}
                    </div>
                  ...
                  <div className="bg-white p-4 rounded-xl border border-gray-200 md:col-span-2">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                      {['Sangat Baik', 'Baik', 'Cukup', 'Kurang'].map(val => (
                        <label key={`kesopanan-${val}`} className={`flex items-center justify-center p-2 rounded cursor-pointer border transition text-center ${formData.kesopanan_petugas === val ? 'bg-green-50 border-green-300 text-green-700 font-bold' : 'border-gray-200 text-gray-600 hover:bg-gray-50'}`}>
                ...
                <div>
                  ...
                  <textarea name="saran" rows={3} value={formData.saran} onChange={handleChange} placeholder="Tuliskan masukan Anda di sini..." className="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 outline-none resize-none"></textarea>
                </div>
              </div>
              ...
              <div className="pt-4 border-t border-gray-100 flex justify-end gap-3">
                ...
                <button type="submit" disabled={loading} className="px-6 py-2 text-sm font-bold text-white bg-green-700 hover:bg-green-800 rounded-lg shadow disabled:opacity-50 transition flex items-center">
                  {loading ? (
